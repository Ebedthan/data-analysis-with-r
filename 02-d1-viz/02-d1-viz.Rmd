...
<p style="font-size: 48px;"><b>L'Analyse de données avec </b><img src="../img/Rlogo.png" width="60px" height="auto"></p>

<h3>La visualisation des données `r emo::ji('graph')`</h3>
<br/>
<div class="author">
  <h3>Anicet Ebou</h3>
</div>
...
```
.question[
Quelles sont les fonctions pour desiner le graphe? Quelles sont les données utilisées pour le graphe? Quelles sont les variables en abscisse et en ordonnée? Que signifie le warning?
]
...
- Ou pour un jeu de données integré en utilisant ?dataset: ` ?starwars`
...
- Il existe des valeurs manquantes dans le jeu de données (d'ou 3 d'entre eux n'ont pas été pris en compte).
...
.instructions[
Comment décririez vous cette relation? Quel autres facteurs permetraient d'expliquer cette relation?
]
...
```{r fig.width=8, fig.height=2.3, warning=FALSE}
ggplot(data = agri_all, mapping = aes(
...
```{r echo=FALSE, fig.height=1.2, fig.width=6}
